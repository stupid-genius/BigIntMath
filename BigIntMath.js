...
Object.defineProperties(biMath, {
	...
	random_bytes: {
		value: (min, max) => {
			const bytes = Math.round((max - min).toString(2).length / 8);
			// console.log(`generating random number (${min}, ${max}) (${bytes} bytes)`);
			let randomBigInt = flyweightCache[0];

			// not sure about the statistics of this, but seems reasonable on the surface
			for(let i = 0; i < bytes; ++i){
				const randomByte = Math.floor(Math.random() * 256);
				randomBigInt = (randomBigInt << flyweightCache[8]) | BigInt(randomByte);
				// console.log(`acc: ${randomBigInt}, generated byte ${randomByte}`);
			}
			// console.log(`bytes: ${randomBigInt.toString(2).length/8}, base: ${randomBigInt}`);
